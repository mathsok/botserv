...
SUPER_ADMIN_ID = int(os.environ.get(“SUPER_ADMIN_ID”, “777785304”))
...
data[“teachers”] = {}
if “teacher_codes” not in data:
data[“teacher_codes”] = {}
return data
return {“teachers”: {}, “teacher_codes”: {}}
...
def get_teacher(tid):
return load_db()[“teachers”].get(str(tid))
...
db = load_db()
existing = set()
for d in db[“teachers”].get(str(tid), {}).get(“students”, {}).values():
for key in (“u_code”, “p_code”, “su_code”):
if d.get(key): existing.add(d[key])
while True:
...
def get_all_teacher_codes(db):
codes = set()
for code in db.get(“teacher_codes”, {}):
codes.add(code)
return codes

def find_teacher_by_login_code(code, db):
“”“Знаходить вчителя за кодом входу”””
return db.get(“teacher_codes”, {}).get(code)
...
menu_teacher_students = ReplyKeyboardMarkup(keyboard=[
[KeyboardButton(text=“➕ Додати учня”), KeyboardButton(text=“📋 Список учнів”)],
[KeyboardButton(text=“💳 Баланси”)],
[KeyboardButton(text=“⬅️ Назад”)]
], resize_keyboard=True)

menu_teacher_lessons = ReplyKeyboardMarkup(keyboard=[
...
menu_student_lessons = ReplyKeyboardMarkup(keyboard=[
...
f”🔔 Нагадування!\nСьогодні о {target_time} у тебе заняття.\nНе забудь підготуватись! 📚”)
except Exception:
pass
...
menu_super_admin = ReplyKeyboardMarkup(keyboard=[
...
if uid == SUPER_ADMIN_ID:
    await message.answer("👑 Панель адміністратора", reply_markup=menu_super_admin)
    return
...
if tid_str in db["teachers"]:
    ...
    await message.answer(f"Вітаю, {t['name']}! Ваша панель керування:", reply_markup=get_webapp_menu(menu_teacher, url))
    return
...
if tid:
    ...
    await message.answer(greeting, reply_markup=get_webapp_menu(get_student_menu(role_menu), url))
    return
...
@dp.message(lambda m: m.from_user.id == SUPER_ADMIN_ID and m.text == “➕ Додати вчителя”)
async def admin_add_teacher(message: types.Message):
user_state[message.from_user.id] = {“state”: “admin_teacher_name”}
await message.answer(“Введіть ім’я вчителя:”, reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=“⬅️ Назад”)]], resize_keyboard=True))
...
teacher_tid = find_teacher_by_login_code(code, db)
if teacher_tid:
    # Bind teacher uid to their account
    tid_str = str(teacher_tid) if not isinstance(teacher_tid, str) else teacher_tid
    if tid_str not in db["teachers"]:
        await message.answer("❌ Вчителя не знайдено. Зверніться до адміністратора.")
        return
    t = db["teachers"][tid_str]
    # Update teacher's telegram id if needed
    db["teachers"][tid_str]["telegram_id"] = uid
    # Also register under their real uid if different
    if str(uid) != tid_str:
        db["teachers"][str(uid)] = db["teachers"][tid_str]
    save_db(db)
    user_state[uid] = None
    url = f"{WEBAPP_URL}?role=admin&tid={uid}"
    await message.answer(
        f"✅ Вітаємо, {t['name']}!\nПредмет: {t.get('subject','')}\n\nВаш кабінет готовий!",
        reply_markup=get_webapp_menu(menu_teacher, url)
    )
    return

# Check if it's a student/parent code
...
if tid:
    ...
    if role_menu == "parent":
        await message.answer(f"✅ Вітаємо! Кабінет учня {sname}.", reply_markup=kb)
    else:
        await message.answer(f"✅ Вітаємо, {sname}!", reply_markup=kb)
    return

await message.answer("❌ Код не знайдено. Спробуйте ще раз або зверніться до адміністратора.")
```

# ─── МЕНЮ ВЧИТЕЛЯ ─────────────────────────────────────────────────────────────

@dp.message(lambda m: m.text == “👥 Керування учнями”)
...
@dp.message(lambda m: m.text == “📋 Список учнів”)
async def teacher_list(message: types.Message):
tid = str(message.from_user.id)
db = load_db()
if tid not in db[“teachers”]: return
...
text = f”👤 *{name}*\n💰 Ціна: {s[‘price’]}₴ | Баланс: {bal_str}\n📱 Учень: {u_status} | 👨‍👩‍👦 Батьки: {p_status} | ⭐ Супер: {su_status}”
...
sessions = [(s[“time”], n) for n, d in students.items() for s in d.get(“sessions”, []) if s[“day”] == today]
sessions.sort()
text = f”📅 Сьогодні ({today}):\n\n” + (”\n”.join([f”{n} — {t}” for t, n in sessions]) or “Немає занять”)
await message.answer(text)
else:
tid, sname, sdata, role = find_student_by_uid(uid)
if sdata:
s = “\n”.join([f”🔹 {s[‘day’]} {s[‘time’]}” for s in sdata.get(“sessions”, [])])
await message.answer(f”📅 Твій розклад:\n{s or ‘Розклад ще не встановлено’}”)
...
@dp.message(lambda m: m.text == “🗓 Розклад на тиждень”)
...
for day in days:
day_sessions = [(s[“time”], n) for n, d in students.items() for s in d.get(“sessions”, []) if s[“day”] == day]
day_sessions.sort()
...
@dp.message(lambda m: m.text == “💳 Баланси”)
...
@dp.message(lambda m: m.text == “✔️ Відмітити заняття”)
...
@dp.message(lambda m: m.text == “📝 Відправити домашнє завдання”)
...
await message.answer(f”📝 Введи текст ДЗ для {name} або надішли фото:”,
reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=“⬅️ Назад”)]], resize_keyboard=True))
...
return
kb = [[KeyboardButton(text=f”📒 Заняття {len(journal)-i}: {e[‘date’]}”)] for i, e in enumerate(journal)]
kb.append([KeyboardButton(text=“⬅️ Назад”)])
text = “📒 *Журнал занять:*\n\n”
for e in journal:
mat_count = len(e.get(“materials”, []))
text += f”📅 {e[‘date’]} — *{e[‘topic’]}*{f’ | 📎{mat_count}’ if mat_count else ‘’}\n”
...
return
kb = [[KeyboardButton(text=f”Т.Журнал {name} #{len(journal)-i}: {e[‘date’]}”)] for i, e in enumerate(reversed(journal))]
kb.append([KeyboardButton(text=“⬅️ Назад”)])
text = f”📒 *Журнал — {name}:*\n\n”
...
@dp.message(lambda m: m.text == “🔗 Корисні посилання”)
...
tid, sname, sdata, role = find_student_by_uid(uid)
if not sdata: return
homework = list(reversed(sdata.get(“homework”) or []))
if not homework:
await message.answer(“📚 Домашніх завдань немає.”)
return
kb = [[KeyboardButton(text=f”ДЗ {len(homework)-i}: {hw[‘date’]} {‘✅’ if hw[‘status’]==‘done’ else ‘🔴’}”)] for i, hw in enumerate(homework)]
kb.append([KeyboardButton(text=“⬅️ Назад”)])
...
save_db(db)
user_state[uid] = None
await message.answer(f”👋 Ти вийшов з кабінету {sname}.\nЩоб увійти знову — введи свій код після /start.”,
...
@dp.message(lambda m: isinstance(user_state.get(m.from_user.id), dict)
...
for t, tdata in db["teachers"].items():
    for s, sd in tdata.get("students", {}).items():
        if s == sname and sd.get("su_id") == uid:
            role = "super"
await message.answer("✅ Запит успішно надіслано!\n\nБаланс буде поповнений, щойно його підтвердить вчитель.",
    reply_markup=get_student_menu(role))
```
...
print(f”[ERROR] {event.exception.**class**.**name**}: {event.exception}”)
import traceback
traceback.print_exc()
...
```
# Ігноруємо стікери, голосові тощо
is_file_state = isinstance(state, dict) and state.get("state") in ("pay_check", "lesson_send_materials", "hw_done_photo", "hw_waiting_text", "waiting_materials")
if not message.text and not message.photo and not message.document:
    if not is_file_state:
        return
...
if uid == SUPER_ADMIN_ID:
    ...
    if isinstance(state, dict) and state.get("state") == "admin_teacher_subject":
        ...
        db2 = load_db()
        existing = get_all_teacher_codes(db2)
        while True:
        ...
        db2["teacher_codes"][code] = teacher_id
        save_db(db2)
        user_state[uid] = None
        await message.answer(
...
if isinstance(state, dict) and state.get("state") == "lesson_send_materials":
    ...
    if message.document:
        ...
        await message.answer(f"📎 {message.document.file_name} ({len(state['materials'])}) додано. Ще або *Готово*.", parse_mode="Markdown")
        return
...
if isinstance(state, dict) and state.get("state") == "waiting_name":
    user_state[uid] = {"state": "waiting_price", "name": message.text, "tid": state["tid"]}
...
if isinstance(state, dict) and state.get("state") == "balance_sub":
    try:
        ...
        if bal < 0:
            ...
            for nid in [i for i in [sdata.get("p_id"), sdata.get("su_id")] if i]:
                try:
...
if isinstance(state, dict) and state.get("state") == "lesson_send_materials" and message.text in ("✅ Готово", "✅ Готово (без матеріалів)"):
    tid = state["tid"]; name = state["name"]; topic = state["topic"]
    date_str = state["date"]; materials = state["materials"]
    user_state[uid] = None
    db2 = load_db()
    sdata = db2["teachers"][tid]["students"][name]
    price = sdata["price"]
...
if isinstance(state, dict) and state.get("state") == "cancel_enter_date":
    date = message.text.strip(); name = state["name"]; tid = state["tid"]
    user_state[uid] = None
...
if isinstance(state, dict) and state.get("state") == "reschedule_enter_new_date":
    new_date = message.text.strip(); old_date = state["old_date"]
    name = state["name"]; tid = state["tid"]
    user_state[uid] = None
...
if isinstance(state, dict) and state.get("state") == "viewing_hw":
    tid = state["tid"]; sname = state["sname"]; hw_id = state["hw_id"]
    db2 = load_db()
...
if isinstance(state, dict) and state.get("state") == "teacher_links_waiting_url":
    ...
    if not url.startswith("http"):
        ...
        return
    tid = state["tid"]; label = state["label"]
    db2 = load_db()
...
# Додавання учня — стан waiting_name
if isinstance(state, dict) and state.get("state") == "managing_student":
