...
async def choose_role(message: types.Message):
    ...
    if message.text == "👨‍🏫 Я вчитель":
        ...
        await message.answer("Як вас звати?", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="⬅️ Назад")]], resize_keyboard=True))
    elif message.text == "🎓 Я учень / батьки":
        user_state[uid] = "waiting_auth_code"
        await message.answer("Введіть код доступу:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="⬅️ Назад")]], resize_keyboard=True))
...
async def teacher_enter_name(message: types.Message):
    ...
    await message.answer("Який предмет викладаєте?")
...
async def teacher_enter_subject(message: types.Message):
    ...
    await message.answer(f"✅ Вітаємо, {name}!\nПредмет: {subject}\n\nВаш кабінет готовий!", reply_markup=get_webapp_menu(menu_teacher, url))
...
async def auth(message: types.Message):
    ...
    if tid:
        ...
        return
    await message.answer("❌ Код не знайдено. Спробуй ще раз.")
